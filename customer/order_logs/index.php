<?php
    ...
    $orderlog = mysqli_query($connection, "SELECT * FROM `order_logs` WHERE `customerId` = '" . $customerId . "' AND `delete_flag` = 0") or die(mysqli_error($connection));
?>
        <div class="flex flex-col gap-5 md:gap-10">
            <h1 class="text-3xl md:text-5xl font-semibold">Order Logs</h1>
            ...
            <div class="flex flex-col gap-10 p-5 md:px-10 md:py-5 rounded-2xl shadow-xl bg-white border border-gray-300">
                ...
                <div class="flex flex-col text-xs md:text-lg md:text-sm gap-2 md:gap-5 overflow-auto md:w-full max-h-[500px]">
                    ...
                    <hr />
                    <?php
                    while ($row = mysqli_fetch_assoc($orderlog)) {
                        $originalDate = $row['order_placed_date'];
                    ...
                    <div class="grid grid-cols-[1fr_2fr_1fr_1fr_1fr_1fr_1fr_1fr] place-items-center font-sans md:w-full w-[700px]">
                        ...
                        <p><?php echo $row['status'] == 0 ? 'Processing' : 'Complete'; ?></p>
