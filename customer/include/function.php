...
//Get State
function get_service($connection)
{
	$data = array();
	$q = isset($_REQUEST["q"]) ? trim($_REQUEST["q"]) : "";
	$categoryId = isset($_REQUEST['categoryId']) ? $_REQUEST['categoryId'] : "";

    $stateResult = mysqli_query($connection,"SELECT * FROM `service` WHERE `categoryId` = '".$categoryId."' AND `service` LIKE '%" . $q . "%' AND `delete` = '0'");
    while($stateRow = mysqli_fetch_array($stateResult))
    {
        $data[] = array("id" => $stateRow["id"], "text" => $stateRow["service"]);
    }
...
function smtp_mailer($to, $subject, $msg)
{
	...
	$mail->IsHTML(true);
	$mail->CharSet = 'UTF-8';
	$mail->Username = "acerhealthcare22@gmail.com";
	$mail->Password = "qotvlwpjvqpzxqre";
	$mail->SetFrom("Lafloridians","acerhealthcare22@gmail.com");
	// $mail->addCC("rayid@bookyourwheels.in");
	// $mail->addCC("support@bookyourwheels.in");
...
{
    $datee =date("d/m/Y",strtotime(str_replace("/", "-", $pickup_date)));
    $currentDateTime =  date($datee.$pickup_time);
    $newDateTimee = date('d/m/Y H:i A', strtotime($currentDateTime));

    $enddatee =date("d/m/Y",strtotime(str_replace("/", "-", $drop_date)));
    $endtripDateTime =  date( $enddatee.$drop_time);
    $endtripdatetimee = date('d/m/Y H:i A', strtotime($endtripDateTime));

    //next day 6 am time
    $currentDateTime_a = $datee.'22:00:00';
    $newDateTime_a = date('d/m/Y H:i A', strtotime($currentDateTime_a));

    $nxt = date('d/m/Y', strtotime($pickup_date. ' + 1 days')); 
    
    $currentDateTime_b = $nxt.'06:00:00';
    $newDateTime_b = date('d/m/Y H:i A', strtotime($currentDateTime_b));
